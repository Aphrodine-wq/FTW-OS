...
export function TaskList() {
  ...
  const deleteTask = (id: string) => {
    ...
    toast({ title: "Task Deleted", description: "Removed from your list." })
  }
          ...
          <div className="space-y-2">
            {activeTasks.length === 0 && completedTasks.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-slate-300">
                <div className="bg-slate-50 p-6 rounded-full mb-4 animate-in zoom-in duration-500">
                  <CheckCircle2 className="h-16 w-16 text-slate-200 stroke-1" />
                </div>
                <h3 className="text-xl font-bold text-slate-900 mb-1">All Caught Up!</h3>
                <p className="text-sm text-slate-500 max-w-[200px] text-center">No pending tasks. Add a new one above to stay productive.</p>
              </div>
            ) : (
