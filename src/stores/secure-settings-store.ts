...
export const useSecureSettings = create<SecureSettingsStore>((set, get) => ({
  ...
  setSecureKey: async (key, value) => {
    ...
    try {
      ...
      console.warn('Vault IPC not available, token only saved to memory/store:', e)
    }
  },
  ...
  getSecureKey: async (key) => {
    ...
    try {
      ...
      console.warn('Vault IPC not available:', e)
      return null
    }
  },
  ...
  loadAllKeys: async () => {
    ...
    try {
      ...
      console.warn('Failed to load secure keys:', e)
    }
  }
}))