...
import { DollarSign, TrendingUp, Users, Target, Zap, Mail, MessageSquare } from 'lucide-react'
import { Invoice, Expense, TimeSession, Task, Subscription, Proposal, Lead, Integration } from '@/types/invoice'
...
export function AnalyticsDashboard({ onNavigate }: AnalyticsDashboardProps) {
  ...
  const loadAnalytics = async () => {
    try {
      ...
      setSessions(data[2] || [])
      setTasks(data[3] || [])
      setSubscriptions(data[4] || [])
      setProposals(data[5] || [])
      setLeads(data[6] || [])
      setIntegrations(data[7] || [])
    } catch (error) {
