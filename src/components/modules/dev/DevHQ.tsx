...
const TerminalView = ({ repo }: { repo: Repo }) => {
    ...
    return <div ref={terminalRef} className="h-full w-full bg-zinc-950 p-2" />
}
...
export function DevHQ() {
    ...
    useEffect(() => {
        if (integrations.githubToken) {
            ...
            // Self-healing: Try to recover token from localStorage if store is empty
            const local = localStorage.getItem('invoiceforge-settings')
            if (local) {
                try {
                    const parsed = JSON.parse(local)
                    if (parsed.integrations?.githubToken) {
                        useSettingsStore.setState(state => ({
                            integrations: { ...state.integrations, ...parsed.integrations }
                        }))
                    }
                } catch (e) {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (!reposRes.ok) throw new Error(`Repo Fetch Failed: ${reposRes.status} ${reposRes.statusText}`)
            const data = await reposRes.json()

            if (Array.isArray(data)) {
                setRepos(data)
                if (data.length === 0) setError("No repositories found (Count: 0). Check token permissions (needs 'repo' scope).")
            } else {
    ...
    const fetchRepoStats = async (repo: Repo) => {
        ...
        try {
            const res = await fetch(`https://api.github.com/repos/${repo.full_name}/languages`, {
                headers: { Authorization: `token ${integrations.githubToken}` }
            })
            if (res.ok) {
