      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)}>
        ...
        <TabsContent value="comparison" className="space-y-6">
          ...
          <div className="flex gap-4 items-center">
            ...
            <Select value={String(taxYear - 1)}>
              <SelectTrigger className="w-[120px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value={String(taxYear - 1)}>{taxYear - 1}</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          {(() => {
            ...
            const prevYearInvoices = invoices.filter(inv => {
              const invDate = new Date(inv.issueDate)
              return invDate.getFullYear() === taxYear - 1 && inv.status === 'paid'
            })

            const prevYearExpenses = expenses.filter(exp => {
              const expDate = new Date(exp.date)
              return expDate.getFullYear() === taxYear - 1
            })

            const prevIncome = prevYearInvoices.reduce((sum, inv) => sum + (inv.total || 0), 0)
            const prevDeductions = prevYearExpenses.reduce((sum, exp) => sum + (exp.amount || 0), 0)
            const prevTaxable = Math.max(0, prevIncome - prevDeductions)
            const prevTax = prevTaxable * ((preferences.defaultTaxRate || 25) / 100)

            const incomeChange = stats.totalIncome - prevIncome
            const incomeChangePercent = prevIncome > 0 ? ((incomeChange / prevIncome) * 100) : 0
            const taxChange = stats.estimatedTax - prevTax
            const taxChangePercent = prevTax > 0 ? ((taxChange / prevTax) * 100) : 0

            const comparisonData = [
              { name: String(taxYear - 1), income: prevIncome, tax: prevTax, deductions: prevDeductions },
              { name: String(taxYear), income: stats.totalIncome, tax: stats.estimatedTax, deductions: stats.totalDeductions }
            ]
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <Card>
                    <CardContent className="p-6">
                      ...
                      <h3 className={`text-2xl font-bold ${incomeChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {incomeChange >= 0 ? '+' : ''}{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(incomeChange)}
                      </h3>
                      <p className="text-xs text-muted-foreground mt-1">
                        {incomeChangePercent >= 0 ? '+' : ''}{incomeChangePercent.toFixed(1)}% from last year
                      </p>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between mb-2">
                        <p className="text-sm text-muted-foreground">Tax Liability Change</p>
                        {taxChange >= 0 ? (
                      ...
                      <h3 className={`text-2xl font-bold ${taxChange >= 0 ? 'text-red-600' : 'text-green-600'}`}>
                        {taxChange >= 0 ? '+' : ''}{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(taxChange)}
                      </h3>
                      <p className="text-xs text-muted-foreground mt-1">
                        {taxChangePercent >= 0 ? '+' : ''}{taxChangePercent.toFixed(1)}% from last year
                      </p>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardContent className="p-6">
                      ...
                      <h3 className="text-2xl font-bold">
                        {stats.totalIncome > 0 ? ((stats.estimatedTax / stats.totalIncome) * 100).toFixed(1) : 0}%
                      </h3>
                      <p className="text-xs text-muted-foreground mt-1">
                        Previous: {prevIncome > 0 ? ((prevTax / prevIncome) * 100).toFixed(1) : 0}%
                      </p>
                    </CardContent>
                  </Card>
                </div>
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Side-by-side analysis of {taxYear - 1} vs {taxYear}</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="h-[300px]">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={comparisonData}>
                          ...
                          <Bar dataKey="deductions" fill="#10b981" name="Deductions" />
                          <Bar dataKey="tax" fill="#ef4444" name="Estimated Tax" />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </CardContent>
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      <div className="grid grid-cols-3 gap-4 pb-2 border-b font-semibold text-sm">
                        <div>Metric</div>
                        <div className="text-right">{taxYear - 1}</div>
                        <div className="text-right">{taxYear}</div>
                      </div>
                      <div className="grid grid-cols-3 gap-4 py-2">
                        <div className="text-sm">Total Income</div>
                        <div className="text-right text-sm">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(prevIncome)}</div>
                        <div className="text-right text-sm font-semibold">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(stats.totalIncome)}</div>
                      </div>
                      <div className="grid grid-cols-3 gap-4 py-2 bg-muted/50">
                        <div className="text-sm">Deductions</div>
                        <div className="text-right text-sm">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(prevDeductions)}</div>
                        <div className="text-right text-sm font-semibold">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(stats.totalDeductions)}</div>
                      </div>
                      <div className="grid grid-cols-3 gap-4 py-2">
                        <div className="text-sm">Taxable Income</div>
                        <div className="text-right text-sm">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(prevTaxable)}</div>
                        <div className="text-right text-sm font-semibold">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(stats.taxableIncome)}</div>
                      </div>
                      <div className="grid grid-cols-3 gap-4 py-2 bg-muted/50 border-t">
                        <div className="text-sm font-semibold">Estimated Tax</div>
                        <div className="text-right text-sm">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(prevTax)}</div>
                        <div className="text-right text-sm font-bold text-blue-600">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(stats.estimatedTax)}</div>
                      </div>
                      <div className="grid grid-cols-3 gap-4 py-2">
                        <div className="text-sm">Invoices Paid</div>
                        <div className="text-right text-sm">{prevYearInvoices.length}</div>
                        <div className="text-right text-sm font-semibold">{stats.yearInvoices}</div>
                      </div>
                      <div className="grid grid-cols-3 gap-4 py-2 bg-muted/50">
                        <div className="text-sm">Expenses</div>
                        <div className="text-right text-sm">{prevYearExpenses.length}</div>
                        <div className="text-right text-sm font-semibold">{stats.yearExpenses}</div>
                      </div>
                    </div>
                  </CardContent>
                </Card>
