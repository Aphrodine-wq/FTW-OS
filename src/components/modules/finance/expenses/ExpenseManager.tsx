...
import { Plus, Search, DollarSign, Calendar, Edit, Trash2, Receipt } from 'lucide-react'
...
export function ExpenseManager() {
  ...
  const loadExpenses = async () => {
    ...
    const parsed = saved.map((e: any) => ({ ...e, date: new Date(e.date) }))
    setExpenses(parsed)
  }
  ...
  const handleSave = async () => {
    ...
    if (currentExpense.id) {
      updatedExpenses = expenses.map(e => e.id === currentExpense.id ? newExpense : e)
    } else {
    ...
    toast({
      title: "Expense Saved",
      description: `${newExpense.description} has been saved.`
    })
  }
